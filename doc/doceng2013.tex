...
project according to the type of change that they represent.  For example, in
...
\item We show that tree based antiunification can for the basis for
highlighting edit patterns in tree structured data.  In particular, we analyze
the tree structure of parsed source code found in version control history to
find edit patterns.


\end{itemize}

What AntiUnification gets us: Antiunification takes two trees and gives us the
common tree structure with ``holes'' and a set of substitutions for each hole
so that we can reconstruct the two input trees. We begin by examining the tree
structure of the substitions.
